    ...
    ui(new Ui::AdminDialog)
{
    ...
    ui->lineEdit->setPlaceholderText(QStringLiteral("输入管理员密码 "));
...
void AdminDialog::on_pbtn_login_clicked()
{
    ...
    {
        QMessageBox::information(this,QStringLiteral("信息 "),QStringLiteral("已登录为管理员。 "));
        ui->lab_loginStatus->setText(QStringLiteral("管理员 "));
    ...
    {
        QMessageBox::warning(this,QStringLiteral("错误 "),QStringLiteral("密码输入错误，请重新输入。 "));
    }
}
void AdminDialog::on_pbtn_logout_clicked()
{
    int a = QMessageBox::question(this,
                                  QStringLiteral("询问 "),
                                  QStringLiteral("退出管理员将导致无法使用日志相关操作，是否确认退出？ "),
                                  QMessageBox::Yes|QMessageBox::No,
                                  QMessageBox::No);
    if(a == QMessageBox::Yes)
    {
        ui->lab_loginStatus->setText(QStringLiteral("未登录 "));
        ui->lineEdit->clear();
