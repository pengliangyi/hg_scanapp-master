...
//    m_scanner = nullptr;
 //   settingsPara = new ScanParam;
    ...
    mes_box->setText(QStringLiteral("扫描中..."));
    mes_box->addButton(QMessageBox::No)->setText(QStringLiteral("取消扫描"));
...
void MainWindow::initUsbDevice()
{
   /* auto scanners = GScanLists::FindAll();
    ...
    if (m_scanner.get() && m_scanner->open())
    {
        ...
        ui->lab_scanCnet->setText(QStringLiteral(""已连接 "));
        ui->lab_scanStatus->setText(QStringLiteral("空闲 "));
        f_connection = true;
...
void MainWindow::loadImgToListview(QString filePath)
{
    ...
    if(globalImg->isNull())
    {
        delete globalImg;
        globalImg = nullptr;
        QMessageBox::information(this,QStringLiteral("信息 "),QStringLiteral("加载图像失败! "));
...
//        QMessageBox::information(this,QStringLiteral("信息 "),QStringLiteral("扫描仪处于计数模式，请先退出计数模式再扫描。 "));
//    }
//    else
//    {
//        this->toolBar->setEnabled(false);
//        this->menuBar()->setEnabled(false);
//        ui->lab_scanStatus->setText(QStringLiteral("扫描中 "));
...
bool MainWindow::writeColorTableFile()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("写colorTable失败，请重试！ "));
        return 0;
    }
...
bool MainWindow::writeGreyTableFile()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("写greyTable失败，请重试！ "));
        return 0;
    }
...
void MainWindow::readColorTableFile()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("读取colorTable失败，请重启应用程序！ "));
        close();
        return;
    }
...
void MainWindow::readGreyTableFile()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("读取greyTable失败，请重启应用程序！ "));
        close();
        return;
    }
...
{
    QFileDialog saveAsDialog(nullptr,QStringLiteral("图像另存为 "),"","(*.jpg);;(*.jpeg);;(*.png);;(*.bmp);;(*.pdf);;(*.tif)");
...
void MainWindow::on_getDevError(int err_code)
{
    ...
    {
    case HAVE_ERROR:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("发生未知错误，扫描中止。"));
        str = "Undefined error,scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case OPEN_COVER:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("机盖被打开！ "));
        str = "Cover opened,scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case NO_FEED:
        QMessageBox::information(this,QStringLiteral("提示 "),QStringLiteral("未检测到文稿，请放入需要扫描的文稿。 "));
        break;
    case FEED_IN_ERROR:
        QMessageBox::information(this,QStringLiteral("提示 "),QStringLiteral("进纸失败，请打开机盖重新整理文稿。 "));
        str = "Feeding failed. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case PAPER_JAM:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("卡纸，请打开机盖清理文稿。 "));
        str = "Paper jammed,scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case DETECT_DOUBLE_FEED:
        QMessageBox::information(this,QStringLiteral("提示 "),QStringLiteral("检测到双张进纸，请取走双张文稿。 "));
        str = "Double feeding,scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case DETECT_STAPLE:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("检测到订书钉，请打开机盖清理文稿。 "));
        str = "Detected staple,scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case PAPER_SKEW:
        QMessageBox::information(this,QStringLiteral("提示 "),QStringLiteral("检测到进纸歪斜，请重新整理文稿。 "));
        str = "Skewed feeding. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case FPGA_ERROR:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("发生固件错误，扫描中止。 "));
        str = "Firmware error(FPGA),scan stopped. ERRCODE="+QString::number(err_code)+" .";
        writeLog(str);
        break;
    case PC_SCAN_BUSY_or_ERROR:
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("计算机繁忙或发生错误。 "));
...
{
    QMessageBox messbox;
    messbox.setWindowTitle(QStringLiteral("询问 "));
...
//                             QStringLiteral("耗材更换提示 "),
//                             QStringLiteral("扫描仪耗材已达到更换值( ") + QString::number(num) + QStringLiteral(")/450000)，请及时更换耗材。\n""(继续使用扫描仪可能出现进纸不顺畅，卡纸，双张等问题)"));
//    }
}
   ...
   /* if(m_scanner->is_open() != f_connection)
    {
        if(f_connection)
        {
            ui->lab_scanCnet->setText(QStringLiteral("未连接 "));
            ui->lab_scanStatus->setText(QStringLiteral("--"));
            QMessageBox::warning(this,
                                 QStringLiteral("连接中断 "),
                                 QStringLiteral("扫描仪连接中断。 "));
...
void MainWindow::on_act_export_multiPdf_triggered()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("选取图像失败，请重试！ "));
        return;
    }
    QString dst = QFileDialog::getSaveFileName(nullptr,QStringLiteral("导出多页pdf文档 "),"","(*.pdf)") + ".pdf";
    FileExport pdf;
    if(filesList.size()==1)
...
void MainWindow::on_act_export_multiTif_triggered()
{
    ...
    QString dst = QFileDialog::getSaveFileName(nullptr,QStringLiteral("导出多页tif文档 "),"","(*.tif)") + ".tif";
    FileExport tif;
    if(filesList.size()==1)
...
void MainWindow::on_act_export_multiOfd_triggered()
{
    ...
    QString dst = QFileDialog::getSaveFileName(nullptr,QStringLiteral("导出多页ofd文档 "),"","(*.ofd)") + ".ofd";
    FileExport ofd;
    if(filesList.size()==1)
...
//    d.setWindowTitle(QStringLiteral("选择批次文件保存的文件夹 "));
//    d.exec();
//    if (!d.result())return;
...
{
    QFileDialog logOutputDialog(nullptr,QStringLiteral("导出日志文档.. "),".","(*.txt)");
...
void MainWindow::on_act_logClr_triggered()
{
    ...
    if(log.open(QIODevice::WriteOnly | QIODevice::Truncate))
    {
        ...
        log.write(str);
        log.close();
        QMessageBox::information(this,QStringLiteral("信息 "),QStringLiteral("日志清空完毕。 "));
    }
    else
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("日志清空失败，请重试！ "));
}
...
void MainWindow::on_act_cacheClr_triggered()
{
    int a = QMessageBox::question(this,
                          QStringLiteral("警告 "),
                          QStringLiteral("清空缓存将关闭所有非本地加载的图像，未保存的图像将会丢失！\n""是否继续清空缓存？ "),
                          QMessageBox::Yes|QMessageBox::No,
                          QMessageBox::No);
...
//            QMessageBox::information(this,QStringLiteral("信息 "),QStringLiteral("缓存已清空！ "));
//        else
//            QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("硬件缓存清空失败！ "));
    }
}
void MainWindow::on_act_help_triggered()
{
    ...
    if(!QFileInfo(file).exists())
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("找不到帮助文档！文档可能已被删除或重命名！ "));
}
