...
void scanSettingsDialog::on_cbox_paperSize_currentIndexChanged(int index)
{
    ...
    if(ui->cbtn_sizeAreaSwitch->isChecked())
    {
        int m = QMessageBox::question(this,
                                      QStringLiteral("重要"),
                                      QStringLiteral("当前文稿尺寸已设置自定义扫描区域。若更换文稿尺寸，将导致已选择自定义扫描区域重置。\n""是否确定要更换文稿尺寸？"));
        if(m == QMessageBox::No)
        {
...
void scanSettingsDialog::on_pBtn_usrSetting_app_clicked()
{
    int a = QMessageBox::question(this,
                          QStringLiteral("询问 "),
                          QStringLiteral("应用已选中的配置，扫描参数将发生更改，是否继续？ "),
    ...
    if(a == QMessageBox::Yes)
    {
        ...
        if(!ini_file.exists())
            QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("加载配置文件失败。配置文件可能已损坏或被删除。 "));
        else
            load_ini(name);
    }
}
...
{
    QInputDialog d(this);
    d.setWindowTitle(QStringLiteral("新增配置"));
    d.setLabelText(QStringLiteral("输入新配置名称"));
    if(d.exec())
    {
        ...
        {
            QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("新增配置文件失败。请检查磁盘剩余空间或当前用户组权限。 "));
            return;
        }
        //增加列表项
        add_usrSetting(name);
...
void scanSettingsDialog::on_pBtn_usrSetting_del_clicked()
{
    int a = QMessageBox::question(this,
                          QStringLiteral("询问 "),
                          QStringLiteral("将永久删除已选中的配置，是否继续？ "),
                          QMessageBox::Yes|QMessageBox::No,
                          QMessageBox::Yes);
    if(a == QMessageBox::Yes)
    {
...
void scanSettingsDialog::on_pBtn_usrSetting_clr_clicked()
{
    int a = QMessageBox::question(this,
                          QStringLiteral("询问 "),
                          QStringLiteral("将永久删除所有已保存的配置，是否继续？ "),
                          QMessageBox::Yes|QMessageBox::No,
                          QMessageBox::Yes);
    if(a == QMessageBox::Yes)
    {
...
void scanSettingsDialog::on_pBtn_usrSetting_exp_clicked()
{
    ...
    if(!file.exists())
    {
        QMessageBox::warning(this,QStringLiteral("警告 "),QStringLiteral("创建配置文件失败。请检查磁盘剩余空间或当前用户组权限。 "));
        return;
    }
    //生成配置文件
    QString name = file.completeBaseName();
...
void scanSettingsDialog::on_pBtn_usrSetting_save_clicked()
{
    int a = QMessageBox::question(this,
                          QStringLiteral("询问 "),
                          QStringLiteral("将覆盖当前已选中的配置，是否继续？ "),
                          QMessageBox::Yes|QMessageBox::No,
                          QMessageBox::Yes);
    if(a == QMessageBox::Yes)
    {
...
void scanSettingsDialog::add_usrSetting(QString name)
{
    ...
    else
    {
        int a = QMessageBox::question(this,
                              QStringLiteral("询问 "),
                              QStringLiteral("已有相同名称配置，是否覆盖同名配置？ "),
                              QMessageBox::Yes|QMessageBox::No,
                              QMessageBox::Yes);
        if(a == QMessageBox::Yes)
        {
...
QString scanSettingsDialog::showIniInputDialog()
{
    ...
    d->setWindowTitle(QStringLiteral("可导入配置列表 "));
    ...
    //创建窗口按钮
    QPushButton *inputBtn = new QPushButton(QStringLiteral("导入 "),d);
    QPushButton *cancelBtn = new QPushButton(QStringLiteral("取消 "),d);
