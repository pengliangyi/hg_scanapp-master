...
{
    rotate_s = new QAction(QStringLiteral("顺时针90° "),this);
    rotate_n = new QAction(QStringLiteral("逆时针旋转90° "),this);
    first_page = new QAction(QStringLiteral("第一页 "),this);
    last_page = new QAction(QStringLiteral("最后一页 "),this);
    delSelect = new QAction(QStringLiteral("删除 "),this);
    merge = new QAction(QStringLiteral("合并为多页文件 "),this);
    save_as = new QAction(QStringLiteral("另存为... "),this);
    close = new QAction(QStringLiteral("关闭 "),this);
...
void ImageList::deleteFunction(QList<QListWidgetItem*> items)
{
    if (items.count() > 0)
    {
        if (QMessageBox::Yes == QMessageBox::question(nullptr, QStringLiteral("关闭图像 "),
            QStringLiteral("关闭%1张图像 ").arg(QString::number(items.count())), QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes))
        {
